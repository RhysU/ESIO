...
if ! [ -x layout2_int ]; then
    echo "layout2_int binary not found or not executable"
    exit 1
fi

if ! which mpiexec > /dev/null ; then
    echo "WARNING: Unable to find mpiexec; skipping layout2_int"
    exit 0
fi
...
do
    for cmd in "mpiexec -np 3 ./layout2_int -p  11 -u  13"
    do
        for dir in "C" "B" "A"
        do
