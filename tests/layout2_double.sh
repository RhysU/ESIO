...
if ! [ -x layout2_double ]; then
    echo "layout2_double binary not found or not executable"
    exit 1
fi

if ! which mpiexec > /dev/null ; then
    echo "WARNING: Unable to find mpiexec; skipping layout2_double"
    exit 0
fi
...
for auxstride in ""                \
                 ...
                 "--auxstride-a=7"
do
    for cmd in "mpiexec -np 1 ./layout2_double -p 11 -u 13" \
               "mpiexec -np 2 ./layout2_double -p  5 -u  7" \
               "mpiexec -np 3 ./layout2_double -p  7 -u  5"
    do
        for dir in "C" "B" "A"
        do
