...
# 
# modifications made to allow for integration with an autotools 
# based build system.
#-------------------------------------------------------------------
#-------------------------------------------------------------------
#
...
sub find_includes {
    ...
    # koomie mod: 7/28/10 - parse ignore_mods
    my @ignore_mods = @_;
	...
	# C preprocessor style includes
	    ...
	    my $test_string = "grvy";
	    if (grep {$_ eq $1 } @ignore_mods) {
		print "# INFO: koomie found module $1 to ignore inside $file \n";
		next;
	    }
	    $use = $1;
...
# Search included file for includes
		...
		$self->{'includes'}{$included} = 1;
                ...
                if ($after !~ /bogus/i) {
		    print  "# WARN: Cannot find file: $included\n";
		}
	    }
	}
    }
...
GetOptions("s","strip","e=s", "f=s", "I=s@", "m=s", "c", "p", "g", "h", "o=s", "a=s","i=s@" ) || die "problem in GetOptions";
#GetOptions("s", "e=s", "f=s", "I:s", "m=s", "c", "p", "g", "h", "o=s", "a=s")
#GetOptions("I:s@") || die "problem in GetOptions";
#getopts("I:s@") || die "problem in GetOptions";
...
if ($opt_strip) {
    $strip_dirs = 1;
    print "# stripping leading directories for src file depends\n";
}
if ($opt_e) {
	$ext = $opt_e;
} else {
...
print "# include search path = @incdirs \n";

# ---------------------------------------------------
# koomie mod. 7/26/2010 - provide mechanism
# to ignore user-provided modules (e.g. those provided
# compiler or 3rd party apps).
# ---------------------------------------------------

if (@opt_i) {
    @ignore_mods = @opt_i;
    print "# INFO: koomie ignoring modules @ignore_mods \n";
}

# ---------------------------------------------------
# koomie mod. 7/26/2010 - send dependencies straight
# to stdout for use inclusion with automake. 
# Disabling default makefile usage.
# ---------------------------------------------------
...
foreach $file (@ARGV) {
    ...
    $sources{$file}->find_includes(@ignore_mods);
}
...
# koomie mod. 7/26/2010 - send dependencies straight
# to stdout for use inclusion with automake. 
# Disabling printing to makefile.
# ---------------------------------------------------
...
if ( !( $cray || $parasoft) ) {
	...
	if($strip_dirs) {
	    print "$modname.$mod_ext: ".basename($object)."\n";
	} else {
